...
A failure **after** the stream started was left alone for a while, on the assumption that reporting it would need a second `RUN_STARTED` mid-stream — which clients do reject. It does not: `RUN_ERROR` is exempt from every ordering guard in both clients, being the protocol's abort event, so the committed stream is simply appended to with one `RUN_ERROR` frame (code `StreamError`). That covers the far more common failure — the provider rejecting the follow-up model call *during* a run, where the alternative is a stream that just stops, with no terminal event for the client to act on. Worth having: a browser client that never sees a terminal event also keeps its `pendingInterrupts`, so the user's next message is rejected as well.
