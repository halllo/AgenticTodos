...
| `backend/ToolApprovalHistoryNormalizer.cs` | The four repairs — see [History replay](#history-replay-why-the-normalizer-is-needed) |
...
| `tests/ToolApprovalHistoryNormalizerTests.cs` | Scrubbing, de-duplication, orphan rejection, idempotency |
...
The persisted request therefore stays "active" while its response was persisted as informational, and every later turn throws. `ToolApprovalHistoryNormalizer` runs on every history load (`ProvideChatHistoryAsync`) with four idempotent repairs:
...
3. **Keep only the newest copy of a request id** — repair 2 de-duplicates the list handed to the model, not the file. The store appends the re-supplied pair as well, so from the resume turn on, *both* copies sit on disk (see any `_full.json` with an approval in it). Repair 1 clears them once the call completes; a history holding the two requests and one response but no result — compaction dropping the tool result is the likeliest way in — matches no repair above, and FICC, pairing one response to one request, throws on the unpaired copy for every remaining turn. The last copy wins: it is the one FICC already flipped to `InformationalOnly`, so the approved call is not re-executed behind the user's back.
4. **Reject orphans** — a request nothing answers (client disconnected, session file lost the response, user just sent a new message) gets a synthetic rejected response appended, so FICC fabricates a failed result and the conversation continues. The console sample can't hit this — its loop forces a y/N answer before accepting input — but a web client abandons approvals routinely.
