...
import { AnnotationLayer, AnnotationMode, PDFWorker, TextLayer } from "pdfjs-dist";
...
// CSP in the MCP host allows blob: but not data: in script-src.
// GlobalWorkerOptions.workerSrc leads pdfjs to inline the worker as data: (via
// vite-plugin-singlefile) and then wrap it in _createCDNWrapper which does
// `await import("data:...")` — still blocked. Instead:
//   1. Inline the worker source via ?raw (no data: URL assigned to workerSrc)
//   2. Create a blob: URL directly from the source text
//   3. Create the Worker as { type: "module" } (pdf.worker.mjs is ESM)
//   4. Hand it to PDFWorker.create({ port }) so pdfjs skips its own worker
//      creation path entirely and uses our already-running Worker.
const _workerBlob = new Blob(
  [pdfWorkerSource.replace(/\/\/# sourceMappingURL=\S+\s*$/m, "")],
  { type: "text/javascript" },
);
const _pdfWorker = PDFWorker.create({
  port: new Worker(URL.createObjectURL(_workerBlob), { type: "module" }),
});
...
// (parseToolResult removed — URL is read directly from tool input arguments)
...
async function fetchChunk(
  ...
  const request = (async (): Promise<RangeResult> => {
    try {
      ...
      const chunk = JSON.parse(textContent.text) as PdfBytesChunk;
...
// Handle tool input — URL comes from the arguments Claude passed to display_pdf
app.ontoolinput = async (params) => {
  const args = params.arguments as { url?: string; height?: number };
  pdfUrl = args.url ?? "";
  if (!pdfUrl) {
    showError("No URL provided to display_pdf");
    return;
  }

  currentPage = loadSavedPage() ?? 1;
